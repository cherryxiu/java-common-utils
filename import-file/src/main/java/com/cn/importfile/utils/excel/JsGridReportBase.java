package com.cn.importfile.utils.excel;

import com.cn.importfile.utils.DateUtil;
...
public class JsGridReportBase {
	...
	public void exportExcel(List<Object> list,String title,String[] hearders,String[] fields,String operatorName) throws Exception{
		...
		if (list.size()>12000) {
			int excelSize = 12000;	//每个Excel文件条数
...
//			InputStream is = res.getInputStream();
			...
			//使用 session 加载器加载文件路径（文件在webapp文件下）
			//String filePath = session.getServletContext().getRealPath("/") + "/excel/module.xls";

			//使用类加载器加载文件路径（文件在resources文件下）
			String filePath = this.getClass().getClassLoader().getResource("excel/module.xls").getPath();
